...
module.exports = (app) => {
  ...
  app.get('/messages', global.asyncWrapper(async (req, res) => {
    ...
    try {
      ...
      const archived = ['1', 'true'].includes(req.query.showArchived || '')
      let filter = {};
      if (archived) {
        filter = { archived: { $eq: true } };
      } else {
        filter = { $or: [{ archived: { $eq: false } }, { archived: { $exists: false } }] };
      }
      ...
      if (pageNumber) {
        ...
        messages = await collection.find(filter)
          .sort({ updatedAt: -1 })
        ...
        messages = await collection.find(filter).sort({ updatedAt: -1 }).toArray();
        response = messages;
      }
